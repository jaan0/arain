    ...
    <div className="fade-in">
      ...
      <div className="grid-metrics">
        <div className="card">
          ...
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>
            {displayProd ? `${displayProd.totalEggs} eggs total` : 'No data'}
          </div>
        </div>
        <div className="card">
      ...
      <div className="card" style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
          ...
          <h3 style={{ fontSize: '1rem' }}>Distribute to Dealer</h3>
        </div>
        <form onSubmit={handleAddDist}>
          ...
          <div className="form-row">
            <div className="form-group" style={{ flex: 1 }}>
              <input type="number" placeholder="Paiti" className="mono" value={newDist.paiti} onChange={e => setNewDist({...newDist, paiti: parseInt(e.target.value)||0})} required style={{ width: '100%' }} />
            </div>
            <div className="form-group" style={{ flex: 1 }}>
              <input type="number" placeholder="Trays" className="mono" value={newDist.trays} onChange={e => setNewDist({...newDist, trays: parseInt(e.target.value)||0})} required style={{ width: '100%' }} />
            </div>
          </div>
          <div className="form-group" style={{ marginTop: '16px' }}>
            <input type="number" placeholder="Rate / Paiti" className="mono" value={newDist.ratePerPaiti} onChange={e => setNewDist({...newDist, ratePerPaiti: parseInt(e.target.value)||0})} required style={{ width: '100%' }} />
          </div>
          <button type="submit" className="btn-outline" style={{ width: '100%', marginTop: '16px' }}>Log Sale</button>
        </form>
      </div>
